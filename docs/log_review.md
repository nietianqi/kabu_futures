...
# Full logged live diagnostics while still replaying only the first 50k books.
python scripts\analyze_micro_evolution.py logs\live_20260428_124855.jsonl --config config\local.json --max-books 50000 --diagnostics-max-rows 0 --no-regime
```
...
- `live_trade_diagnostics`: live closed-trade PnL reconstructed from `live_trade_closed` or, for older logs, from position/exit order status events. Review `trades`, `net_pnl_ticks`, `net_pnl_yen`, `by_symbol`, `by_exit_reason`, and `entry_price_mismatches`.
- `jump_diagnostics`: `jump_detected` rejects split by `symbol`, `session_phase`, and `spread_ticks`. Use it to decide whether an open-session or TOPIX-specific jump spike is real before changing any threshold.
- `--diagnostics-max-rows`: controls logged live diagnostics separately from replay. By default it follows `--max-books` for fast smoke checks; pass `0` for full-log diagnostics.
...
- Heartbeat fields `live_trades_count`, `live_pnl_ticks`, `live_pnl_yen`, `live_win_rate`, and `live_avg_pnl_ticks`: these are the live closed-trade totals. They are separate from paper PnL, which stays zero in pure live mode.
- `live_order_status.order_status.execution_price`: should come from a filled kabu `Details` row with `ExecutionID` when available. If `live_position_detected.metadata.entry_price_mismatch` is true, inspect the mismatch sample in `live_trade_diagnostics` before interpreting PnL.
...
- In live logs with actual fills, prefer `live_trade_diagnostics` and heartbeat live PnL over paper fields. `paper_pnl_*` intentionally remains zero when the run is live-only.
...
- If `entry_price_mismatch` appears, do not assume the top-level kabu order `Price` is the fill. Filled `Details` with `ExecutionID` are treated as the auditable execution source, while the synced `/positions` price remains the state-machine entry price until manually reviewed.
