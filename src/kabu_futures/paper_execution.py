...
class PaperMinuteExecutor:
    """Paper-only lifecycle manager for minute-level strategy signals."""
    ...
    def _maybe_arm_breakeven(self, exit_price: float) -> None:
        ...
        if trade is None or trade.breakeven_armed or trade.stop_distance_ticks <= 0:
            return
        if self._pnl_ticks(trade.direction, trade.entry_price, exit_price) < trade.stop_distance_ticks * self.config.minute_engine.breakeven_after_r:
            return
