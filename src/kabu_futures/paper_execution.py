...
ExecutionEventType = Literal["paper_entry", "paper_exit", "paper_pending", "paper_cancel", "execution_reject"]
...
class PaperExecutionController:
    ...
    def on_book(self, book: OrderBook, features: BookFeatures | None = None) -> list[ExecutionEvent]:
        if self.trade_mode == "observe":
            return []
        return self.micro_executor.on_book(book, features)

    def heartbeat_metadata(self) -> dict[str, object]:
        stats = self.micro_executor.stats()
        return {
            "paper_position": self.micro_executor.position_summary() if self.trade_mode == "paper" else None,
            "paper_pending_order": self.micro_executor.pending_summary() if self.trade_mode == "paper" else None,
            "paper_trades": stats.trades,
            "paper_pnl_ticks": round(stats.pnl_ticks, 4),
            "paper_pnl_yen": round(stats.pnl_yen, 2),
            "paper_pending_orders": stats.pending_orders,
        }
